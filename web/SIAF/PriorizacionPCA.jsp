<%-- 
    Document   : PriorizacionPCA
    Created on : 23/11/2017, 02:37:36 PM
    Author     : H-URBINA-M
--%>

<!DOCTYPE html>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<script type="text/javascript">
    $(document).ready(function () {
        var theme = getTheme();
        $("#div_Titulo").jqxExpander({theme: theme, width: '100%'});
        $("#cbo_Periodo").jqxComboBox({theme: theme, autoOpen: true, promptText: "Seleccione", width: 100, dropDownWidth: 150, height: 20});
        $("#cbo_Presupuesto").jqxComboBox({theme: theme, autoOpen: true, promptText: "Seleccione", width: 200, dropDownWidth: 350, height: 20});
        $("#cbo_GenericaGasto").jqxComboBox({theme: theme, autoOpen: true, promptText: "Seleccione", width: 250, dropDownWidth: 350, height: 20});
        var fecha = new Date();
        $("#cbo_Periodo").jqxComboBox('selectItem', fecha.getFullYear());
        $('#cbo_Periodo').on('change', function () {
            $("#cbo_GenericaGasto").jqxDropDownList('clear');
            fn_cargarGenericaGasto("#cbo_GenericaGasto", {mode: 'genericaGasto', periodo: $("#cbo_Periodo").val(), presupuesto: $("#cbo_Presupuesto").val()});
        });
        $('#cbo_Presupuesto').on('change', function () {
            $("#cbo_GenericaGasto").jqxDropDownList('clear');
            fn_cargarGenericaGasto("#cbo_GenericaGasto", {mode: 'genericaGasto', periodo: $("#cbo_Periodo").val(), presupuesto: $("#cbo_Presupuesto").val()});
        });
        $('#cbo_GenericaGasto').on('change', function () {
            fn_CargarBusqueda();
        });
        fn_cargarGenericaGasto("#cbo_GenericaGasto", {mode: 'genericaGasto', periodo: $("#cbo_Periodo").val(), presupuesto: $("#cbo_Presupuesto").val()});
    });
    function fn_CargarBusqueda() {
        var periodo = $("#cbo_Periodo").val();
        var presupuesto = $("#cbo_Presupuesto").val();
        var genericaGasto = $("#cbo_GenericaGasto").val();
        var msg = "";
        if (msg === "")
            msg = fn_validaCombos('#cbo_GenericaGasto', "Seleccione la Generica de Gasto.");
        if (msg === "") {
            $("#div_GrillaPrincipal").remove();
            $("#div_VentanaPrincipal").remove();
            $("#div_VentanaDetalle").remove();
            $("#div_VentanaDetalleRegistro").remove();
            var $contenidoAjax = $('#div_Detalle').html('<img src="../Imagenes/Fondos/cargando.gif">');
            $.ajax({
                type: "GET",
                url: "../PriorizacionPCA",
                data: {mode: "G", periodo: periodo, presupuesto: presupuesto, genericaGasto: genericaGasto},
                success: function (data) {
                    $contenidoAjax.html(data);
                }
            });
        } else {
            $.alert({
                theme: 'material',
                title: 'AVISO DEL SISTEMA',
                content: msg,
                animation: 'zoom',
                closeAnimation: 'zoom',
                type: 'orange',
                typeAnimated: true
            });
        }
    }
    function fn_cargarGenericaGasto(obj, datos) {
        $.ajax({
            type: "GET",
            url: "../CombosAjax",
            data: datos,
            success: function (data) {
                var source = {
                    localdata: data,
                    datatype: "json",
                    datafields:
                            [
                                {name: 'descripcion', type: 'string'},
                                {name: 'codigo', type: 'string'}
                            ],
                    async: true
                };
                var dataAdapter = new $.jqx.dataAdapter(source);
                $(obj).jqxComboBox({source: dataAdapter, placeHolder: "Seleccione", displayMember: "descripcion", valueMember: "codigo"});
            }
        });
    }
</script>
<div style="border: none;" id='div_Titulo'>
    <div class="jqx-hideborder">PRIORIZACIÓN DE PCA</div>
    <div>
        <div id="div_Cabecera">
            <table class="navy">
                <tbody>
                    <tr>
                        <td >Periodo : </td>
                        <td>
                            <select id="cbo_Periodo" name="cbo_Periodo">
                                <c:forEach var="a" items="${objPeriodo}">
                                    <option value="${a.codigo}">${a.codigo}</option>
                                </c:forEach>
                            </select>
                        </td>
                        <td>PPTO. : </td>
                        <td>
                            <select id="cbo_Presupuesto" name="cbo_Presupuesto">
                                <c:forEach var="b" items="${objPresupuesto}">
                                    <option value="${b.codigo}">${b.descripcion}</option>
                                </c:forEach>
                            </select>
                        </td>
                        <td>Generica de Gasto : </td>
                        <td>
                            <select id="cbo_GenericaGasto" name="cbo_GenericaGasto">
                                <option value="0">Seleccione</option>                                
                            </select>
                        </td>
                        <td><a href="javascript: fn_CargarBusqueda();" ><img src="../Imagenes/Botones/refresh42.gif" alt="Buscar Datos" name="imgrefresh" width="30" height="28" border="0" id="imgrefresh"></a></td>
                        <td><a href="../Login/Principal.jsp" target="_parent"><img src="../Imagenes/Botones/exit42.gif" alt="Salir de pantalla" name="imgexit" width="30" height="28"  border="0" id="imgexit" /></a></td>
                    </tr>                    
                </tbody>
            </table>
        </div>
        <div id="div_Detalle" class="maincen"></div>
    </div>
</div>