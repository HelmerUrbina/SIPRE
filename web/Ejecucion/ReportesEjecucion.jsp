...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#cbo_MesInicial").jqxComboBox({theme: theme, autoOpen: true, promptText: "Seleccione", width: 100, dropDownWidth: 150, height: 20});
        $("#cbo_MesFinal").jqxComboBox({theme: theme, autoOpen: true, promptText: "Seleccione", width: 100, dropDownWidth: 150, height: 20});
        $("#cbo_Generica").jqxComboBox({theme: theme, autoOpen: true, promptText: "Seleccione", width: 300, dropDownWidth: 350, height: 20});
        $("#div_Fecha").jqxDateTimeInput({culture: 'es-PE', animationType: 'fade', width: 150, height: 20});
        var fecha = new Date();
        $("#cbo_Periodo").jqxComboBox('selectItem', fecha.getFullYear());
        $("#cbo_MesFinal").jqxComboBox('selectIndex', fecha.getMonth());
    ...
    function fn_CargarBusqueda() {
        ...
        if (msg === "") {
            ...
            var generica=$("#cbo_Generica").val();
            $("#div_Principal").remove();
            var $contenidoAjax = $('#div_Detalle').html('<img src="../Imagenes/Fondos/cargando.gif">');
            $.ajax({
                type: "POST",
                url: "../ReportesEjecucion",
                data: {mode: "G", periodo: periodo, presupuesto: presupuesto, unidadOperativa: unidadOperativa,generica: generica},
...
<div style="border: none;" id='div_Titulo'>
    <div class="jqx-hideborder">REPORTES DE EJECUCIÓN PRESUPUESTAL</div>
    <div>
        <div id="div_Cabecera">
            <table class="navy">
                <tbody>
                    ...
                    <tr>
                        <td>Mes Inicial : </td>
                        <td>
                            <select id="cbo_MesInicial" name="cbo_MesInicial">
                                <c:forEach var="a" items="${objMes}">
                        ...
                        <td>Mes Final : </td>
                        <td>
                            <select id="cbo_MesFinal" name="cbo_MesFinal">
                                <c:forEach var="a" items="${objMes}">
                                    <option value="${a.codigo}">${a.descripcion}</option>
                                </c:forEach>
                            </select>
