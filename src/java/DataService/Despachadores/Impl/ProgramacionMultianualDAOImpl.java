...
public class ProgramacionMultianualDAOImpl implements ProgramacionMultianualDAO {
    ...
    public BeanProgramacionPresupuestal getSaldoUtilidadEnteGenerador(BeanProgramacionPresupuestal objBeanProgramacion, String usuario) {
        sql = "SELECT SUM(UTILIDAD_A-IMPORTE_A) AS SALDO_A, "
                ...
                + "FROM (SELECT "
                + "SUM(ROUND((CASE TO_NUMBER(EG.CPERIODO_CODIGO) WHEN TO_NUMBER(EG.CANIO_REGISTRO) THEN ((NPROG_MULTI_ENTE_RECAUDACION-NPROG_MULTI_ENTE_COSTO)*(EI.NPORCENTAJE_RDR_UO/100)+NPROG_MULTI_ENTE_COSTO) ELSE 0 END))) AS UTILIDAD_A, "
                + "SUM(ROUND((CASE TO_NUMBER(EG.CPERIODO_CODIGO) WHEN TO_NUMBER(EG.CANIO_REGISTRO+1) THEN ((NPROG_MULTI_ENTE_RECAUDACION-NPROG_MULTI_ENTE_COSTO)*(EI.NPORCENTAJE_RDR_UO/100)+NPROG_MULTI_ENTE_COSTO) ELSE 0 END))) AS UTILIDAD_B, "
                + "SUM(ROUND((CASE TO_NUMBER(EG.CPERIODO_CODIGO) WHEN TO_NUMBER(EG.CANIO_REGISTRO+2) THEN ((NPROG_MULTI_ENTE_RECAUDACION-NPROG_MULTI_ENTE_COSTO)*(EI.NPORCENTAJE_RDR_UO/100)+NPROG_MULTI_ENTE_COSTO) ELSE 0 END))) AS UTILIDAD_C, "
                + "0 AS IMPORTE_A, 0 AS IMPORTE_B, 0 AS IMPORTE_C "
