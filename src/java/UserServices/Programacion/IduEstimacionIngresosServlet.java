 ...
 * @author H-URBINA-M
 */
@WebServlet(name = "IduEstimacionIngresosServlet", urlPatterns = {"/IduEstimacionIngresos"})
public class IduEstimacionIngresosServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        objBnEstimacionIngresos.setDescripcion(request.getParameter("descripcion"));
        objBnEstimacionIngresos.setCadenaIngreso(request.getParameter("cadenaIngreso"));
        objBnEstimacionIngresos.setImpuesto(request.getParameter("impuestos"));
        objBnEstimacionIngresos.setImporteUnidadOperativa(Utiles.checkDouble(request.getParameter("importeUnidadOperativa")));
        objBnEstimacionIngresos.setImporteUnidadEjecutora(Utiles.checkDouble(request.getParameter("importeUnidadEjecutora")));
        objDsEstimacionIngresos = new EstimacionIngresosDAOImpl(objConnection);
        // EJECUTAMOS EL PROCEDIMIENTO SEGUN EL MODO QUE SE ESTA TRABAJANDO
        int k = objDsEstimacionIngresos.iduEstimacionIngresos(objBnEstimacionIngresos, objUsuario.getUsuario());
        if (k != 0) {
        } else {
